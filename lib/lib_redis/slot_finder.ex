defmodule LibRedis.SlotFinder do
  @moduledoc """
  ...
  def hash_slot(key) do
    bytes = key |> to_charlist() |> extract_hashable_portion

    res =
      Enum.reduce(bytes, 0, fn element, crc ->
        bxor(crc <<< 8 &&& 0xFFFF, Enum.at(@crc16tab, bxor(crc >>> 8, element &&& 0x00FF)))
      end)
  ...
  defp extract_hashable_portion(key_bytes) do
    ...
    if end_found &&
      ...
      Enum.slice(key_bytes, start_idx + 1, end_idx - start_idx - 1)
    else
      key_bytes
    end
  end
