...
#   @enforce_keys ~w(name urls password pool_size refresh_interval_ms)a
...
#         {:ok, _} = LibRedis.Standalone.start_link(pool: pool)
#         [{host, pool} | acc]
#       end)
...
#   defp get_or_new_client(host, state) do
#     state.clients
...
#         pool =
#           LibRedis.Standalone.new(
#             name: :"#{state.name}-#{length(state.clients) + 1}",
#             url: url_with_password(host, state.password),
#             pool_size: state.pool_size
#           )

#         {:ok, _} = LibRedis.Standalone.start_link(pool: pool)
#         {pool, %{state | clients: [pool | state.clients]}}
...
#       slot ->
#         host = slot.master.ip <> ":" <> Integer.to_string(slot.master.port)
#         {client, state} = get_or_new_client(host, state)
